...
$_POST['first_name'] = trim($_POST['first_name']);
	try {
		...
		if (strlen($_POST['first_name']) >2 && strlen($_POST['first_name']) <20 ) {
			...
			throw new Exception('Firstname must be 2-19 characters!');
		}
	}
	catch(Exception $e) {
		...
		<div class="testbox">
			...
			<?php
				...
				<!DOCTYPE html>
				  ...
				  <label id="icon" for="name"><i class="icon-envelope "></i></label>
				  <input type="email" name="email" id="name" placeholder="Email" value="'.$u_email.'"  required/>
				  <label id="icon" for="name"><i class="icon-envelope "></i></label>
				  <input type="text" name="mobile" id="name" placeholder="Phone" onkeypress="return (event.charCode !=8 && event.charCode ==0 || (event.charCode >= 48 && event.charCode <= 57))" value="'.$u_mobile.'" required/>
