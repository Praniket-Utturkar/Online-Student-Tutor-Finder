...
if (isset($_REQUEST['uid'])) {
	...
	if ($get_tutor_name != null) {
		...
		// Handle the case when $get_tutor_name is null
		$medium="";
		$sub="";
		$cls="";
		$plocation_db="";
...
if (isset($_POST['updatetutioninfo'])) {
	...
	try {
		if(($user == $_REQUEST['uid']) && ($utype_db == "teacher")) {
			$sublist = isset($_POST['sub_list']) ? implode(',', $_POST['sub_list']) : '';
			$classlist = isset($_POST['class_list']) ? implode(',', $_POST['class_list']) : '';
			$mediumlist = isset($_POST['medium_list']) ? implode(',', $_POST['medium_list']) : '';

			// Update the tutor information in the database
			$result4 = $con->query("INSERT INTO tutor (t_id,prefer_sub,class,medium,inst_name,salary,prefer_location) VALUES ('$user','$sublist','$classlist','$mediumlist','$uinst_db','$_POST[sal_range]','$_POST[location]')");
			
			if ($result4) {
				// If the insertion is successful, delete the previous tutor information
				$result = $con->query("DELETE FROM tutor WHERE id='$id_db'");
			}
			
			// Success message
			$success_message = '<div class="signupform_content"><h2><font face="bookman">Post successful!</font></h2><div class="signupform_text" style="font-size: 18px; text-align: center;"></div></div>';
